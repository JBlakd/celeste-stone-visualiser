...
export function PerMaterialConfigurator({
  ...
  const modelPath = modelPreset.path.startsWith("http")
    ? modelPreset.path
  ...
  const getExplodeVec = useCallback(
  ...
  const resetMesh = useCallback(
    (mesh: THREE.Object3D, info: MeshInfo) => {
  ...
  useEffect(() => {
    ...
    scene.traverse((child) => {
      ...
      const materialConfig = modelPreset.materials.find(
        (m) => m.id === materialName,
      );
      ...
      if (materialConfig) {
        ...
        const currentColor =
          colors[materialConfig.id] || materialConfig.defaultColor;
        ...
        const isButtonMaterial =
          materialConfig.id === "actionButtonsColor" ||
          materialConfig.id === "directionalButtonsColor";
        ...
        if (materialName === "frontPlateOutlineColor") {
          ...
          applyPosition(
            info?.explodeVec ?? getExplodeVec("frontPlateOutlineColor"),
          );
        } else {
