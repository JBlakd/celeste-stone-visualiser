...
import {
  collectMeshes,
  computeAutoFit,
  computeRadialDirections,
} from "../../three/meshUtils";
...
export function FBXConfigurator({ colors, modelPreset, explodeAmount }: Props) {
  ...
  const textureUrls = useMemo(() => {
    ...
    return {
      ...
      roughnessMap: texturePaths.roughness
        ? base + texturePaths.roughness
        : undefined,
      metalnessMap: texturePaths.metallic
        ? base + texturePaths.metallic
        : undefined,
    };
  }, [
  ...
  const presentUrls = Object.values(textureUrls).filter((u): u is string =>
    Boolean(u),
  );
  ...
  useEffect(() => {
    ...
    const bodyConfig =
      modelPreset.materials.find((m) => m.id === "body") ??
      modelPreset.materials[0];
    const currentColor = bodyConfig
      ? colors[bodyConfig.id] || bodyConfig.defaultColor
      : "#ffffff";
