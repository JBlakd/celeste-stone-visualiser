...
export function Scene() {
  ...
  const [colorsByModel, setColorsByModel] = useState<
    Record<string, DynamicColors>
  >(() => {
    const init: Record<string, DynamicColors> = {};
    MODEL_PRESETS.forEach((p) => {
      init[p.id] = buildDefaultColors(p);
    });
    return init;
  });

  const colors =
    colorsByModel[currentModel.id] ?? buildDefaultColors(currentModel);
  ...
  const handleResetColors = useCallback(() => {
    setColorsByModel((prev) => ({
      ...prev,
      [currentModel.id]: buildDefaultColors(currentModel),
    }));
  }, [currentModel]);
  ...
  return (
    <div style={{ width: "100vw", height: "100vh" }}>
      ...
      <button
        ...
        aria-label={
          colorPanelOpen ? "Hide color controls" : "Show color controls"
        }
